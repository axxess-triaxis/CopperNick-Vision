...
- Missing values are blank cells, not a sentinel like -9999 (that convention
  is used by IBTrACS's other file formats, not CSV).
"""
    ...
    for _, row in subset.iterrows():
        points.append(
            CycloneTrackPoint(
                ...
                lat=float(row["LAT"]),
                lon=float(row["LON"]),
                wind_knots=float(row["USA_WIND"]) if pd.notna(row.get("USA_WIND")) else None,
                pressure_mb=float(row["USA_PRES"]) if pd.notna(row.get("USA_PRES")) else None,
            )
        )
    return points
